...
if (file_exists($conf->birddy->multidir_output[$conf->entity] . '/run/birddydaemon.pid'))
{
	...
	$(function() {
		$.get('<?php echo dol_buildpath('/birddy/tpl/birddy.chat.tpl.php', 1); ?>', function(html) {
			...
			if (typeof birddySocket != "undefined")
			{
			...
			function minimizeChat(step) {
				...
				if (step == 1) {
					...
					$("#birddy-reduce-window").removeClass('fa-minus-square-o').addClass('fa-plus-square-o');
				}
			}
			...
			function maximizeChat(step) {
				...
				if (step == 1) {
					...
					$("#birddy-reduce-window").removeClass('fa-plus-square-o').addClass('fa-minus-square-o');
				}
			}
